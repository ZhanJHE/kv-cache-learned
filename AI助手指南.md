...
**当前状态**：代码已实现（阶段 1→3 完整源码 + 阶段 4 预留），可直接上传至 AutoDL 等 Linux CUDA 环境运行。原始蓝图仍保留在 `实验规划.md` 中供对照。
...
├── AI助手指南.md            # 本文件
├── 项目简介.md              # 项目简介（中文）
├── 实验规划.md              # 完整实验规划文档（~35 KB，含原始蓝图）
├── 部署指南.md              # AutoDL 云端部署与环境配置指南
├── LICENSE                  # GPL v3
├── autodl_cloud/
...
当前仓库**无自动化单元测试**。`实验规划.md` 中提供了实验检查清单（Checklist）作为人工验证标准：
...
- **若用户要求实现代码**：核心源码已全部生成，请按 `实验规划.md` 检查是否遗漏；如需修改特征维度或模型结构，请同步更新 `predictor/` 与 `simulator/` 中的对应实现，并保持 `实验规划.md` 一致。
...
- **若用户要求修改设计**：任何对模块划分、特征维度、模型结构的改动都应同步更新 `实验规划.md`，以保持文档与实现一致。
